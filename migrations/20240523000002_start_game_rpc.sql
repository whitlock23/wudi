...
declare
  ...
  v_count int;
begin
  -- Check permission (room owner) - skipped for now, trust client logic or add check
  ...
  for v_player in select * from public.room_players where room_id = p_room_id loop
    ...
    insert into public.game_players (game_id, user_id, hand_cards, cards_count, is_landlord)
    values (v_game_id, v_player.user_id, v_hand, v_count, v_is_landlord);
  end loop;
